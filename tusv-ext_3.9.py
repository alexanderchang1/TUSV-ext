...
# Translated to Python 3.9 by Alexander Chang
...
# TBD
import model.solver as sv

# TBD
import help.file_manager as fm      # sanitizes file and directory arguments

# TBD
import help.generate_matrices as gm # gets F, Q, G, A, H from .vcf files

# TBD
import help.printer as pt

# TBD
import help.vcf_help as vh
import pickle

# TBD
from model.snv_matching import snv_assign
...
def unmix(in_dir, out_dir, n, c_max, lamb1, lamb2, num_restarts, num_cd_iters, num_processors, time_limit, metadata_fname, num_seg_subsamples, should_overide_lambdas, const, sv_ub, only_leaf, collapse, threshold, multi_num_clones=False):
    print("unmix")
...
def write_to_files(d, l_g, U, C, E, R, W, W_SV, W_SNV, W_SNV_UNSAMPLED, W_con, obj_val, F_phasing_full, F_unsampled_phasing_full, org_indices, writer, E_pre, R_pre, W_pre, B, A):
    ...
    r = int((l_g_2r - l_g) / 2)  # Ensure integer division
    n, _ = C.shape
    ...
    for fname in fnames:
        open(fname, 'a').close()  # Assuming fm.touch() creates a file if it doesn't exist
    ...
    with open(fnames[2], 'w') as f:
        f.write(dot.source)  # write tree T in dot format
    dot.format = 'svg'
    dot.render(d + 'T')  # display tree T in .svg
    dot = to_dot(E_pre, R_pre, W_pre)
    with open(fnames[14], 'w') as f:
        f.write(dot.source)  # write tree T in dot format
    dot.format = 'svg'
    dot.render(d + 'T_pre')
...
def check_valid_input(Q, Q_unsampled, G, A, H, F_phasing_full, F_unsampled_phasing_full):
    ...
    A_msg = 'There is an issue with input integer matrices A and H (indicating the number of reads mapped to each mated breakpoint and the number of total reads mapping to a breakpoint). The number of mated reads must be less or equal to the total reads and both should be non negative.'
    print(Q[np.where(np.sum(Q, axis=1) != 1)])
    sys.stdout.flush()

    raise_if(not np.all(np.sum(Q, 1) == 1), Q_msg)
    raise_if(not np.all(np.sum(Q_unsampled, 1) == 1), Q_unsampled_msg)

    print(np.where(np.sum(G, 0) != 2), np.where(np.sum(G, 0) != 2))
    raise_if(not np.all(np.sum(G, 0) == 2) or not np.all(np.sum(G, 1) == 2), G_msg)
    for i in range(0, l):
        for j in range(0, l):
            raise_if(G[i, j] != G[j, i], G_msg)
            raise_if(i == j and G[i, j] != 1, G_msg)

    for p in range(0, m):
        for b in range(0, l):
            raise_if(A[p, b] < 0 or A[p, b] > H[p, b], A_msg)
    return Q, Q_unsampled, G, A, H, F_phasing_full, F_unsampled_phasing_full

def raise_if(should_raise, msg):
    if should_raise:
        raise Exception(msg)
...
def set_non_dir_args(parser):
    ...
    parser.add_argument('-t', '--cord_desc_iters', required=True, type=lambda x: fm.valid_int_in_range(parser, x, 1, MAX_CORD_DESC_ITERS), help='maximum number of coordinate descent iterations for each initialization of U')
    ...
    parser.add_argument('-m', '--time_limit', type=int, help='maximum time (in seconds) allowed for a single iteration of the coordinate descent algorithm')
    ...
    parser.add_argument('-b', '--override_lambdas', action='store_true', help='specify this argument if you would like the parameters lambda1 and lambda2 to be set proportional to the input data set')
